...
export const OrderTrackingWidget = ({ orderId }) => {
    ...
    // ── No data yet: show the trigger button (your existing design) ──────────
    if (!trackingData) {
        return (
            <div style={{ marginTop: 14, display: "flex", justifyContent: "flex-end" }}>
                ...
                {error && (
                    <p style={{ fontFamily: "'Jost', sans-serif", fontSize: "12px", color: "#C4727A", marginLeft: 12, alignSelf: "center" }}>
                        {error}
                    </p>
                )}
            </div>
    ...
    const { currentStatus, awbCode, courierName, scans = [] } = trackingData;
        ...
        <div style={{ backgroundColor: "#F9F3EB", overflow: "hidden", marginTop: 14 }}>
            ...
            >
                <div>
                    ...
                    <p style={{ fontFamily: "'Jost', sans-serif", fontSize: "13px", fontWeight: 600, color: "#AB721E", margin: 0 }}>
                        {currentStatus || "Unknown"}
                    </p>
                </div>
                <span style={{ color: "#8C7B6B", fontSize: 16, transform: expanded ? "rotate(90deg)" : "rotate(0deg)", transition: "transform 0.2s" }}>
                    ›
                </span>
            </div>
                ...
                <div style={{ padding: "16px", borderTop: "1px solid #E8DDD0" }}>
                    ...
                    {scans.length > 0 && (
                        <div style={{ marginTop: 16, paddingTop: 16, borderTop: "1px solid #F5E6D0" }}>
                            <p style={{ fontFamily: "'Jost', sans-serif", fontSize: "10px", fontWeight: 700, letterSpacing: "0.12em", color: "#8C7B6B", marginBottom: 12 }}>
                                TRACKING TIMELINE
                            </p>
                                ...
                                {[...scans].reverse().slice(0, 5).map((s, idx) => (
                                    <div key={idx} style={{ display: "flex", gap: 10 }}>
                                        ...
                                        <div style={{ flex: 1, minWidth: 0 }}>
                                            <p style={{ fontFamily: "'Jost', sans-serif", fontSize: "12px", fontWeight: 600, color: "#1f1b15", margin: "0 0 2px" }}>
                                                {s.status}
                                            </p>
                                            <p style={{ fontFamily: "'Jost', sans-serif", fontSize: "11px", color: "#8C7B6B", margin: 0 }}>
                                                {s.location && <span>{s.location} · </span>}
                                                <span>{s.date || "Date unknown"}</span>
                                            </p>
                                        </div>
                                    </div>
                                ))}
                            ...
                            {scans.length > 5 && (
                                <p style={{ fontFamily: "'Jost', sans-serif", fontSize: "11px", color: "#AB721E", margin: "10px 0 0", textAlign: "center" }}>
                                    +{scans.length - 5} more updates
                                </p>
                            )}
                        </div>
                    )}
                    ...
                    <button
                        ...
                        style={{
                            ...
                            border: "1px solid #E8DDD0", borderRadius: 6,
