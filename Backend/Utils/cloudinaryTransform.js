...
// ─── ImageKit URL Transform Helpers ────────────────────────────────────────

export const imagekitTransform = (url, transformation = "f-auto,q-75") => {
  if (!url) return url;
  const separator = url.includes("?") ? "&" : "?";
  return `${url}${separator}tr=${transformation}`;
};

export const imagekitProductImage = (url, size = "desktop") => {
  const transforms = {
    // ✅ Super tiny LQIP - loads in milliseconds
    lqip: "f-auto,q-30,w-20,h-26,bl-30",

    // Thumbnail for sidebar
    thumbnail: "f-auto,q-50,w-100,h-140,c-at_max",

    mobile: "f-auto,q-65,w-500,c-at_max",
    tablet: "f-auto,q-70,w-800,c-at_max",
    desktop: "f-auto,q-75,w-1000,c-at_max",
  };

  return imagekitTransform(url, transforms[size] || transforms.desktop);
};

// ✅ Generate LQIP placeholder (real image, super optimized)
// export const generateLQIPPlaceholder = (url) => {
//   return imagekitProductImage(url, "lqip");
// };

// export const generateImageSrcSet = (url) => {
//   if (!url) return "";
//   return `
// ${imagekitProductImage(url, "mobile")} 500w,
// ${imagekitProductImage(url, "tablet")} 800w,
// ${imagekitProductImage(url, "desktop")} 1000w
// `.trim();
// };

// ✅ Generate inline base64 blur placeholder (no extra fetch!)
export const generateBlurPlaceholder = (url) => {
  // Tiny inline blur SVG - loads instantly, provider-agnostic, unchanged
  return "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 1000 1300'%3E%3Cfilter id='b'%3E%3CfeGaussianBlur stdDeviation='20'/%3E%3C/filter%3E%3Crect width='1000' height='1300' fill='%23E8DDD0' filter='url(%23b)'/%3E%3C/svg%3E";
};